...
unsigned char* iexe() {
	...
	while (*cip != I_EOL) {
        ...
        case I_SAVE:
            cip++;
            err = save_bas( iexp(), (uint32_t)listbuf, (uint32_t)getlast(), iexp());
            if (err) break;
            break;
        case I_LOAD:
            cip++;
            lineno = iexp(); // get Flash Page
            uint8_t autof = load_bas(lineno, (uint32_t)listbuf);
            if(autof==1) {irun(); return 0;}
            break;
        case I_MON:
            cip++;
            monitor();
...
void basic(){
	...
	while(1){
	    if(autof==1){
	        strcpy(lbuf, "LOAD 0"); autof = 0; err = 0;
	    } else {
	        c_putch('>');// Prompt
	        c_gets(); // Input 1 line
	    }
